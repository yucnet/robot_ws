...
Transform():it(n)
{
    ...
    this->publisher = this->n.advertise<geometry_msgs::PointStamped>("stampedPoint",1000,this);
    ...
    this->camera_matrix << 615.1227416992188, 0.000000,  327.1946716308594,
        ...
        if(this->corners.size() == 0)
            {
                ...
                for(int i=0;i< this->corners[0].size();i++)
                    {
                        double d = cv_ptr->image.at<unsigned short>(corners[0][i].x,corners[0][i].y)/1000.00;
                        cout<<d<<endl;
...
void Transform::callbackImage(const sensor_msgs::ImageConstPtr& msg)
    {   
        ...
        cv::Mat paper = cv_ptr->image.clone();
        this->getMarker(paper,marker_center,0);
        ...
        if(!marker_image.empty())
            {
                ...
                if(ids.size()>0 )
                {
                    ...
                    for(int i = 0;i<corners[0].size();i++)
                        {
                            //cout<<"("<<corners[0][i].x<<","<<corners[0][i].y<<")"<<endl;
                        }
                      cv::aruco::drawDetectedMarkers(marker_image, corners, ids);

                this->getMarkerCoordinate(corners,ids,marker_center);
                    cv::aruco::estimatePoseSingleMarkers(corners,0.145, this->camera_matrix, dist_coeffs, rvecs, tvecs);
                    ...
                    if(rvecs.empty()&&tvecs.empty())
                        ...
                        {
                            cv::aruco::drawAxis(marker_image, this->camera_matrix, dist_coeffs ,rvecs, tvecs, 0.1); 
                        }  
